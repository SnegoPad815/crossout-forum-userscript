...
var avatarsDatabaseURL = 'https://raw.githubusercontent.com/SnegoPad815/crossout-forum-userscript-and-restyle/master/coverDB.user.js';
  ...
  if (window.location.search.substr(2,7)=="profile") {
    ...
    /*и если нет фона*/
      if (ProfileHead.getElementsByClassName('ipsCoverPhoto_photo').length===0) {
        ...
        addProfileCover(ProfileHead,ProfileID,0);
      }
    }
}

function findHoverCards(){
  ...
  for(var i = 0; i < target.length; i++){
    target[i].addEventListener("mouseenter", function( event ) { 
      setTimeout(function() {
        ...
        /*если фон есть, не менять. + защита от нескольих установок*/
        if (ProfileHead[0].getElementsByClassName("ipsCoverPhoto_container").length===0){
          ...
          addProfileCover(ProfileHead[0], ProfileID,1);            
          }
      ...
      /*HeaderSize*/
      /*0 - нормальный размер (в профиле) */
      /*1 - маленький (в всплывающем окне)*/
function addProfileCover(ProfileHead, ProfileID, HeaderSize) {
  ...
  for(var i = 0; i < covers.length; i++) {
    ...
    if (ProfileID==covers[i].id)
    {
      ...
      if (HeaderSize===0){
        ...
        offset = Math.round(covers[i].top-covers[i].top/100*66); //смещение - 66%
      }
      
      var ht = '<img style="opacity: 1; position: absolute; left: 0px; top: -'+offset+'px;" src="'+url+'" class="ipsCoverPhoto_photo" alt="">';
